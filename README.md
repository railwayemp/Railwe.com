...
2. **During the 30 days**: they keep using the app. A floating 💳 button
   is visible while logged in — tapping it opens a payment screen with a
   UPI QR code / "Pay via UPI App" button.
   ⚠️ You still need to set your real UPI ID: open `index.html`, search
   for `UPI_ID = 'yourupi@bank'` and replace it with your actual UPI
   address (e.g. `yourname@okhdfcbank`). Until you do, the button shows
   "coming soon" instead of a working QR/link.
3. **When the trial ends (or whenever you decide)**: open `admin.html`,
   find them under **Active (trial or paid)**, and tap **Block**. Their
   `allowedUsers` entry is set to `false`.
4. **What the blocked user sees**: next time they open the app, instead
   ...
   it against your own Razorpay/UPI payment notification.
5. **After they pay**: open `admin.html`, find them under **Blocked /
   needs payment** (their submitted UTR shows on the card), confirm
   against your payment notification, and tap **✓ Mark Paid & Approve**.
   They're back in immediately — no trial timer this time, marked as
   paid permanently (until you Block them again, e.g. for a new billing
   cycle).
...
Approve in `admin.html`, which takes a couple of seconds per user.
